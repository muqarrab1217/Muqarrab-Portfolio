...
const ProjectCard = ({
  ...
  const isInternal = projectLink.startsWith('/');

  if (isInternal) {
    return (
      <Link
        to={projectLink}
        className={"relative block p-4 rounded-2xl bg-zinc-800 hover:bg-zinc-700/50 active:bg-zinc-700/60 ring-1 ring-inset ring-zinc-50/5 transition-colors h-full " + classes}
      >
        <figure className="img-box w-full h-72 rounded-lg mb-4 overflow-hidden">
          {imgSrc.includes('.mp4') ? (
            <video
              src={imgSrc}
              alt={title}
